...
class PipelineScheduler:
    ...
    def _calculate_next_run(self, schedule: ScheduleConfig) -> Optional[datetime]:
        ...
        if schedule.type == ScheduleType.ONCE:
            ...
            interval = schedule.interval or 1
            next_run = start_time.replace(hour=0, minute=0, second=0, microsecond=0)
            while next_run <= now:
            ...
            interval = schedule.interval or 1
            next_run = start_time.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
            while next_run <= now:
                # Add months
    ...
    def _parse_cron_expression(self, cron_expr: str, from_time: datetime) -> Optional[datetime]:
        ...
        try:
            ...
            logger.warning(f"Complex cron expression not supported: {cron_expr}")
            return None
    ...
    async def _check_and_execute_jobs(self):
        ...
        for job in list(self.jobs.values()):
            ...
            if job.next_run <= now:
                ...
                # Execute pipeline
                try:
                    logger.info(f"Executing scheduled pipeline: {job.pipeline_name}")
                    execution = await engine.execute_pipeline(
                        pipeline,
                        parameters={"triggered_by": "scheduler"}
                    )
                    ...
                    logger.error(f"Error executing scheduled pipeline {job.pipeline_name}: {e}")
